    ...
    steps: [
      {
        trigger: "pay",
        label: "用户支付订单",
        detail: "用户通过微信/支付宝完成付款，系统自动确认收款",
      },
      {
        trigger: "ship",
        label: "商家确认发货",
        detail: "仓库拣货完成，物流单号已录入，商品已发出",
      },
      {
        trigger: "receive",
        label: "用户确认收货",
        detail: "物流显示已签收，用户点击确认收货",
      },
      {
        trigger: "complete",
        label: "订单完成",
        detail: "订单状态变为已完成，交易成功",
      },
    ],
  },
  refund_approval: {
    ...
    steps: [
      {
        trigger: "submit",
        label: "提交客服审核",
        detail: "退款申请已提交，等待客服审核材料",
      },
      {
        trigger: "cs_approve",
        label: "客服审核通过",
        detail: "客服确认退款原因合理，提交经理审批",
      },
      {
        trigger: "manager_approve",
        label: "经理审批通过",
        detail: "经理批准退款，款项将退回用户账户",
      },
    ],
  },
  product_approval: {
    ...
    steps: [
      {
        trigger: "submit",
        label: "提交审核",
        detail: "商品信息已完善，提交管理员审核",
      },
      {
        trigger: "approve",
        label: "审核通过上架",
        detail: "管理员审核通过，商品在前端商城可见",
      },
    ],
  },
};
...
export async function POST(req: NextRequest) {
  try {
    ...
    const customer = await prisma.user.findFirst({
      where: { email: "customer@ecomflow.com" },
    });

    if (!product || !customer) {
      return NextResponse.json({ error: "演示数据未就绪" }, { status: 500 });
    }
